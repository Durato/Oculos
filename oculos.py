...
def main():
    ...
    while True:
        ...
        if primeiro:
            ...
            print(f"Frame OK: {w}x{h}, brilho médio={brilho:.1f} (0=preto, 255=branco)")
            cv2.imwrite("debug_frame.jpg", frame)
            print("Salvei debug_frame.jpg — abre pra ver se a câmera está capturando.")
            primeiro = False

        resultados = modelo(frame, verbose=False, conf=0.5)[0]
        anotado = resultados.plot()
        cv2.imshow("Oculos - YOLO", anotado)
